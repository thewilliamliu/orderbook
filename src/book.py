...
class OrderBook:
    def __init__(self) -> None:
        # Price level -> FIFO queue of resting orders (time priority).
        ...
        # Monotonically increasing counters for IDs and event timestamps.
        self._next_id = 1
        self._clock = 0

    # Highest price a buyer is currently willing to pay.
    def best_bid(self) -> int | None:
        self._prune(self._bid_heap, self._bids, negated=True)
        return -self._bid_heap[0] if self._bid_heap else None

    # Lowest price a seller is currently willing to accept.
    def best_ask(self) -> int | None:
        self._prune(self._ask_heap, self._asks, negated=False)
    ...
    # Submit a limit order. Returns (assigned_id, trades); any unfilled
    # remainder rests on the book under assigned_id.
    def submit_limit(
        self, side: Side, price: int, quantity: int
    ...
    # Submit a market order. Returns (assigned_id, trades); any unfilled
    # remainder is discarded (immediate-or-cancel).
    def submit_market(
        self, side: Side, quantity: int
    ) -> tuple[int, list[Trade]]:
        if quantity <= 0:
            raise ValueError("quantity must be positive")
        order_id = self._assign_id()
    ...
    # Walk the opposite side, filling qty best-price-first at resting prices.
    # limit is the incoming price, or None for a market order.
    def _match(
